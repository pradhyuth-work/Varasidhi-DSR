...
-- paid_at: server-set only, never accepted from the client. Backfilled from
-- created_at for existing rows (same moment, under today's single-step
-- payment flow) rather than defaulting to now() at migration time, which
-- would incorrectly collapse historical payments to "today" and break
-- oldest-first report sorting.
...
-- Outward leg of the inventory movement log: one row per product per
-- load-in save, recording stock leaving the warehouse for a route. Mirrors
-- `purchases` (inward) and `stock_returns` (inward, route back to warehouse)
-- so the three tables together give a full inward/outward audit trail.
